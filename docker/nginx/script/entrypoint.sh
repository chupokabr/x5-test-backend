...
echo "PROXY_PASS_HOST=${PROXY_PASS_HOST:=localhost}, PROXY_PASS_PORT=${PROXY_PASS_PORT:=80}"

#replace PROXY_PASS_HOST
sed -i "s|PROXY_PASS_HOST|${PROXY_PASS_HOST}|g" /etc/nginx/conf.d/*.conf
sed -i "s|PROXY_PASS_PORT|${PROXY_PASS_PORT}|g" /etc/nginx/conf.d/*.conf
